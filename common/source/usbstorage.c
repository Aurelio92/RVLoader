...
/* Slightly adjusted nintendont port by FIX94
   Further modified by Aurelio to implement a different reset procedure */
...
static s32 __usbstorage_clearerrors(usbstorage_handle *dev, u8 lun)
{
    ...
    USBStorageOGC_StartStop(dev, lun, 0, 1, 0);
...
{
    USBStorageOGC_StartStop(dev, __lun, 0, 0, 0);
...
{
    if(USB_Initialize() < 0 || USBStorageOGC_Initialize() < 0)
        return false;
