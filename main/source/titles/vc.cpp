...
std::vector<GameContainer> vcGames;
static lwp_t vcCoversThreadHandle;
static u8* vcCoversThreadStack;
mutex_t vcCoversMutex;

static const char _gamesRegions[] = {'E', 'P', 'J'}; //Implementing every region slows down loading times

static void* loadVCCoversThread(void* arg) {
    char tempPath[PATH_MAX];
    char tempID[7];

    for (auto& game : vcGames) {
...
static void loadVCCovers() {
    LWP_MutexInit(&vcCoversMutex, false);
    vcCoversThreadStack = (u8*)memalign(32, THREAD_STACK_SIZE);
    LWP_CreateThread(&vcCoversThreadHandle, loadVCCoversThread, NULL, vcCoversThreadStack, THREAD_STACK_SIZE, 30);
}
...
void addVCGames() {
    ...
    while ((dirp = readdir(dp)) != NULL) {
        ...
        sprintf(tempPath, "/vc/%s", dirp->d_name);
    ...
    std::sort(vcGames.begin(), vcGames.end(), GameContainer::compare);
    loadVCCovers();
}
